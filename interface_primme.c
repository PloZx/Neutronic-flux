...
int primme(int primme_n, int *primme_ia, int *primme_ja, double *primme_a, 
           int nev, double *evals, double *evecs, double *resNorms, primme_target eigtarget)

/*
   But
   ===
   Calcule les nev valeurs propres les plus hautes/basses de la matrice A de dimensions
   primme_n x primme_n stoquée dans le format CSR à l'aide des vecteurs
...
*/
{
    ...
    err = dprimme (evals, evecs, resNorms, &primme);
    if(err){
