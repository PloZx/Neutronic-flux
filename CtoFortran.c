 ...
 */
int csrC_to_upperCSRFortran(int n,int *ia,int *ja,double *a,int **IA_F_out,int **JA_F_out,double **A_F_out,int *nnz_F_out) {
    ...
    // Allocation
    int *IA_F = (int*)malloc((n + 1) * sizeof(int));
    int *JA_F = (int*)malloc((size_t)nnz_upper * sizeof(int));
    ...
    for (int i = 0; i < n; ++i) {
        int write_pos = IA_F[i];  // début de la ligne i en 0-based
        for (int k = ia[i]; k < ia[i+1]; ++k) {
            int j = ja[k];
            if (j >= i) {
                JA_F[write_pos] = (int)j;   // 0-based
                A_F[write_pos]  = a[k];
                write_pos++;
            }
        }
